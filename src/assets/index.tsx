...
async function getArticles() {
  return await sbFetch("articles?select=*&order=submitted_at.desc");
}
...
async function deleteArticleDB(id) {
  await sbFetch("articles?id=eq." + id, { method: "DELETE" });
}
...
function Chip(props) {
  return (
    <span style={{ background: props.color + "33", color: props.color, border: "1px solid " + props.color + "55", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4 }}>
      {props.text}
    </span>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.navy, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <div style={{ background: C.card, borderRadius: 20, padding: "40px 34px", width: "100%", maxWidth: 370, border: "1px solid #1a2a5e" }}>
        ...
        <input value={user} onChange={function(e) { setUser(e.target.value); }} placeholder="Enter username" autoComplete="off" style={Object.assign({}, inputStyle, { border: "1px solid " + (err ? C.red : "#1a2a5e") })} />
        <label style={{ color: C.gray, fontSize: 12, display: "block", marginBottom: 5 }}>Password</label>
        <div style={{ position: "relative", marginBottom: 10 }}>
          <input value={pass} onChange={function(e) { setPass(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter") { submit(); } }} type={show ? "text" : "password"} placeholder="Enter password" autoComplete="off" style={Object.assign({}, inputStyle, { marginBottom: 0, paddingRight: 40, border: "1px solid " + (err ? C.red : "#1a2a5e") })} />
          <span onClick={function() { setShow(!show); }} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", cursor: "pointer", color: C.gray }}>{show ? "🙈" : "👁️"}</span>
        </div>
        ...
        <button onClick={submit} style={{ width: "100%", padding: "12px", background: "linear-gradient(90deg,#7B2FFF,#00D4FF)", border: "none", borderRadius: 10, color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer" }}>🔐 Sign In</button>
...
export default function CMS() {
  ...
  async function setStatus(id, status) {
    await updateArticle(id, { status: status });
    await loadArticles();
    showToast(
  ...
  async function flagToggle(id) {
    const article = articles.find(function(a) { return a.id === id; });
    if (!article) { return; }
    await updateArticle(id, { flagged: !article.flagged });
    await loadArticles();
  }
  ...
  function delWriter(id) {
    ...
    showToast("Writer removed.", C.red);
  }
  ...
  const filtered = articles.filter(function(a) {
    const ms = a.title && a.title.toLowerCase().indexOf(search.toLowerCase()) !== -1 || a.author && a.author.toLowerCase().indexOf(search.toLowerCase()) !== -1;
    const mf = fStatus === "all" || a.status === fStatus;
    return ms && mf;
  });

  const navTabs = [["dashboard","📊","Dashboard"],["submissions","📥","Submissions"],["publish","✍️","Publish"],["writers","👥","Writers"],["analytics","📈","Analytics"]];
    ...
    <div style={{ minHeight: "100vh", background: C.navy, fontFamily: "Inter, sans-serif", display: "flex" }}>
      <div style={{ width: 190, background: C.card, borderRight: "1px solid #1a2a5e", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ padding: "10px 8px", borderTop: "1px solid #1a2a5e" }}>
          ...
          <button onClick={function() { loadArticles(); showToast("Refreshed!"); }} style={{ width: "100%", padding: "8px", background: "transparent", border: "1px solid #1a2a5e", borderRadius: 8, color: C.gray, cursor: "pointer", fontSize: 12, marginBottom: 6 }}>🔄 Refresh</button>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 22 }}>
        ...
        {loading ? <div style={{ textAlign: "center", padding: 40, color: C.gray }}>⏳ Loading from database...</div> : null}

        {!loading && tab === "dashboard" && (
          <div>
            <h2 style={{ color: C.white, marginBottom: 4 }}>📊 Dashboard</h2>
            <p style={{ color: C.gray, fontSize: 13, marginBottom: 18 }}>Welcome back, Admin. Data live from Supabase.</p>
            ...
            <div style={{ background: C.card, borderRadius: 14, padding: 18, border: "1px solid #1a2a5e", marginBottom: 16 }}>
              ...
              {articles.length === 0
                ? <Empty icon="📭" title="No submissions yet." sub="Articles submitted on the main website will appear here." />
                : articles.slice(0, 5).map(function(a) {
                    return (
                      <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid #1a2a5e", cursor: "pointer" }} onClick={function() { setTab("submissions"); setOpenId(a.id); }}>
                        <div>
                          <div style={{ color: C.white, fontSize: 13, fontWeight: 600 }}>{a.title && a.title.length > 50 ? a.title.slice(0, 50) + "..." : a.title}</div>
                          <div style={{ color: C.gray, fontSize: 11 }}>{a.author} · {a.submitted_at ? a.submitted_at.slice(0, 10) : "Just now"}</div>
                        </div>
                        <Chip text={a.status ? a.status.toUpperCase() : "PENDING"} color={a.status === "approved" ? C.green : a.status === "rejected" ? C.red : C.gold} />
                      </div>
                    );
                  })
              }
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              {["all","pending","approved","rejected"].map(function(s) {
                return <button key={s} onClick={function() { setFStatus(s); }} style={{ padding: "8px 12px", background: fStatus === s ? C.accent : C.card, border: "1px solid #1a2a5e", borderRadius: 8, color: fStatus === s ? "#000" : C.gray, cursor: "pointer", fontSize: 12, fontWeight: 600, textTransform: "capitalize" }}>{s}</button>;
              })}
            </div>
            {filtered.length === 0
              ? <div style={{ background: C.card, borderRadius: 14, border: "1px solid #1a2a5e" }}><Empty icon="📭" title="No articles found." sub="Submissions from krynolux.work will appear here automatically." /></div>
                  ...
                  return (
                    <div key={a.id} onClick={function() { setOpenId(open ? null : a.id); }} style={{ background: open ? C.cardHover : C.card, borderRadius: 12, padding: "13px 15px", marginBottom: 8, border: "1px solid " + (a.flagged ? C.red : open ? C.accent : "#1a2a5e"), cursor: "pointer" }}>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 5 }}>
                        ...
                        <Chip text={a.status ? a.status.toUpperCase() : "PENDING"} color={a.status === "approved" ? C.green : a.status === "rejected" ? C.red : C.gold} />
                        <Chip text={a.category || "Uncategorized"} color={C.accent} />
                      </div>
                      <div style={{ color: C.white, fontWeight: 700, fontSize: 14, marginBottom: 3 }}>{a.title}</div>
                      <div style={{ color: C.gray, fontSize: 12 }}>by {a.author} · {a.school} · {a.submitted_at ? a.submitted_at.slice(0, 10) : "Just now"}</div>
                      {open ? (
                        <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #1a2a5e" }}>
                          <p style={{ color: C.gray, fontSize: 13, lineHeight: 1.6, marginBottom: 12 }}>{a.body || "No body provided."}</p>
        ...
        {!loading && tab === "publish" && (
          <div>
            ...
            <p style={{ color: C.gray, fontSize: 13, marginBottom: 18 }}>Publish directly to krynolux.work via Supabase.</p>
                  ...
                  return (
                    <div key={w.id} style={{ background: C.card, borderRadius: 12, padding: "13px 16px", marginBottom: 8, border: "1px solid " + (w.status === "suspended" ? C.red : "#1a2a5e"), display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 7 }}>
                        <button onClick={function() { suspendToggle(w.id); }} style={{ padding: "6px 12px", background: "transparent", border: "1px solid " + (w.status === "active" ? C.red : C.green), borderRadius: 7, color: w.status === "active" ? C.red : C.green, cursor: "pointer", fontSize: 12, fontWeight: 700 }}>{w.status === "active" ? "Suspend" : "Restore"}</button>
        ...
        {!loading && tab === "analytics" && (
          <div>
            ...
            <p style={{ color: C.gray, fontSize: 13, marginBottom: 18 }}>Live data from Supabase.</p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
            ...
            <div style={{ background: C.card, borderRadius: 14, padding: 18, border: "1px solid #1a2a5e", marginBottom: 14 }}>
              ...
              {articles.filter(function(a) { return a.status === "approved"; }).length === 0
                ? <Empty icon="📊" title="No published articles yet." sub="Approve articles in Submissions to see them here." />
                : articles.filter(function(a) { return a.status === "approved"; }).sort(function(a, b) { return (b.views || 0) - (a.views || 0); }).slice(0, 5).map(function(a, i) {
                    return (
                      <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: "1px solid #1a2a5e" }}>
                        ...
                        <div style={{ color: C.accent, fontWeight: 700, fontSize: 13 }}>{(a.views || 0)} views</div>
                      </div>
                    );
                  })
              }
